...
public class SysScheduleDao extends BaseDao {
    ...
    public static List<SysSchedule> selectTableByIdList(List<Integer> idList) throws SQLException {
        return selectTableByIdList(DEFAULT_BEAN, idList);
    }

    public static List<SysSchedule> selectAllTable() throws SQLException {
        return selectAllTable(DEFAULT_BEAN);
    }

    @SuppressWarnings("unchecked")
    public static PageBean<SysSchedule> selectTableForCustomPage(SysScheduleVO bean) throws Exception {
    ...
    public static Map<String, SysSchedule> selectUriScheduleMap() throws SQLException {
        ...
        try {
            rs = executeSelectReturnResultSet(sql, null);
            while (rs.next()) {
    ...
    public static boolean checkScheduleUrlUseable(String scheduleUrl, Integer id) throws SQLException {
        Integer existId = executeSelectReturnInteger("SELECT ss.id FROM sys_schedule ss WHERE ss.schedule_url = ? LIMIT 1", List.of(scheduleUrl));
        return existId == null || existId.equals(id);
    }

    public static int updateLastExecuteTime(String scheduleUrl, Integer lastExecuteTime) throws SQLException {
        return executeUpdate("UPDATE sys_schedule ss SET ss.last_execute_time = ? WHERE ss.schedule_url = ?", List.of(lastExecuteTime, scheduleUrl));
    }
}